...
const [BILI_JCT, SESSDATA, DEDEUSERID, DEDEUSERID__CKMD5] =
...
async function getFansCount() {
  try {
    const res = await axios.request({
      url: "https://api.bilibili.com/x/web-interface/card?mid=" + DEDEUSERID,
    });
...
async function getLastPubDistance() {
  ...
  const res = await axios.request({
    url:
      ...
      DEDEUSERID +
      "&pn=1&ps=1&order=pubdate&jsonp=jsonp",
  });

  try {
    return (
      diffDays(
        new Date(res.data.data.list.vlist[0].created * 1000),
        new Date()
      ) + ""
    );
  } catch (err) {
...
async function painting() {
  ...
  for (let i = 0; i < col; i++) {
    for (let j = 0; j < row; j++) {
      face = await loadImage(infos[n++].face);
      offsetX = w * i;
      offsetY = w * j;
      offsetX = ctx.drawImage(face, offsetX, offsetY, 64, 64);
    }
  }
